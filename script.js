...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function flamesFromCount(count){
    ...
    while(letters.length > 1){
      // elimination index: (count % letters.length) - 1, but handle modulo zero case
      idx = (count % letters.length) - 1;
      if(idx >= 0){
        // remove at idx
        letters.splice(idx, 1);
        // rotate so next start is idx (same index)
        letters = letters.slice(idx).concat(letters.slice(0, idx));
      } else {
        // idx == -1 then remove last
        letters.pop();
      }
    }
    return letters[0];
  ...
  form.addEventListener('submit', (e) => {
    ...
    // persist last result briefly
    try { localStorage.setItem('flames:last', JSON.stringify({ a: aRaw, b: bRaw, r: letter })); } catch(e){}
  });
